...
public:
	...
	void DetiGalia(double g) { kel_galia = g; }
	void DetiTalpa(int t)	{ talpa = t; }
...
int main() {
	...
	cout << "Aukščiausio studento amžius: " << Auksciausio_amzius(studentai, kiek)
		<< "\nJauniausio studento ūgis: " << Jauniausio_ugis(studentai, kiek)
		<< " m.\n\n\n\n";
	IvedimasApieLifta(liftas);
	cout << "Liftas studentus pakels per " << PerKiekKartuPakils(liftas, studentai, kiek)
		<< " kartus.\n\n\n\n";

	double laik = liftas.ImtiKelGalia();
	double galia = liftas.ImtiKelGalia() * 2;
	liftas.DetiGalia(galia);
	if (PerKiekKartuPakils(liftas, studentai, kiek) <= 1)
		cout << "Studentai padvigubinus lifto kel. galią bus pakelti per 1 kartą.\n\n";
	else
		cout << "Padvigubinus lifto kel. galią studentų liftas nepakeltų per 1 kartą.\n\n";

	liftas.DetiGalia(laik);
	int talpa = liftas.ImtiTalpa() * 2;
	liftas.DetiTalpa(talpa);
	if (PerKiekKartuPakils(liftas, studentai, kiek) <= 1)
		cout << "Studentai padvigubinus lifto talpą bus pakelti per 1 kartą.\n\n";
	else
		cout << "Padvigubinus lifto talpą studentų liftas nepakeltų per 1 kartą.\n\n";
